...
describe('tier 0 — new player', () => {
  it('shows Safe + Heist as links; Security, History and Create visibly locked with conditions', () => {
    ...
    expect(screen.getByRole('link', { name: /Security — locked\. Complete your first heist/ })).toHaveAttribute('aria-disabled', 'true');
    expect(screen.getByRole('link', { name: /History — locked/ })).toHaveAttribute('aria-disabled', 'true');
    expect(screen.getByRole('link', { name: /Create — locked\. Complete 5 heists/ })).toHaveAttribute('aria-disabled', 'true');
  });
...
describe('tier progression through play', () => {
  it('unlocks Security + History after the first completed heist (win or lose) and announces it', async () => {
    ...
    // Create still locked.
    expect(screen.getByRole('link', { name: /Create — locked/ })).toBeInTheDocument();
  });
